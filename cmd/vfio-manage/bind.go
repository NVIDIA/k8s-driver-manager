	...
	c := cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{
				...
				Aliases:     []string{"l"},
				Destination: &m.options.loadVFIOModule,
				Value:       true,
...
func (m bindCommand) bindAll() error {
	...
	for _, dev := range devices {
		...
		if err := m.nvpciLib.BindToVFIODriver(dev, m.options.loadVFIOModule); err != nil {
...
func (m bindCommand) bindDevice() error {
	...
	// (cdesiniotis) ideally this should be replaced by a call to nvdev.BindToVFIODriver()
	if err := m.nvpciLib.BindToVFIODriver(nvdev, m.options.loadVFIOModule); err != nil {
