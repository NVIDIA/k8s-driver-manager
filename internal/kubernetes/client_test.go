...
func TestRetryOnAnyErrorRetriesUntilSuccess(t *testing.T) {
	attempts := 0
	err := RetryOnAnyError(context.Background(), discardLogger(), fastBackoff(5), "reach the api server", func() error {
		attempts++
		if attempts < 3 {
...
func TestRetryOnAnyErrorReturnsLastErrorWhenExhausted(t *testing.T) {
	attempts := 0
	err := RetryOnAnyError(context.Background(), discardLogger(), fastBackoff(3), "reach the api server", func() error {
		attempts++
...
// cordonWithRetry and uncordonWithRetry mirror how cmd/driver-manager composes
// the retry policy with the client call. The retry lives at the call site, so
// that composition is what these tests exercise.
func cordonWithRetry(c *Client, backoff wait.Backoff, nodeName string) error {
	return RetryOnAnyError(context.Background(), discardLogger(), backoff, "cordon node "+nodeName, func() error {
		return c.CordonNode(nodeName)
	})
}

func uncordonWithRetry(c *Client, backoff wait.Backoff, nodeName string) error {
	return RetryOnAnyError(context.Background(), discardLogger(), backoff, "uncordon node "+nodeName, func() error {
		return c.UncordonNode(nodeName)
	})
}
...
func TestCordonNodeRetriesUntilSuccess(t *testing.T) {
	...
	require.NoError(t, cordonWithRetry(c, fastBackoff(5), "gpu-node"))
...
func TestCordonNodeReturnsErrorWhenRetriesExhausted(t *testing.T) {
	...
	err := cordonWithRetry(c, fastBackoff(3), "gpu-node")
	require.Error(t, err)
...
func TestUncordonNodeRetriesUntilSuccess(t *testing.T) {
	...
	require.NoError(t, uncordonWithRetry(c, fastBackoff(5), "gpu-node"))
	require.False(t, api.cordoned())
	require.Equal(t, 4, api.requestCount())
}
...
func TestUncordonNodeReturnsErrorWhenRetriesExhausted(t *testing.T) {
	...
	err := uncordonWithRetry(c, fastBackoff(3), "gpu-node")
	require.Error(t, err)
	require.True(t, api.cordoned())
	require.Equal(t, 3, api.requestCount())
}
...
func TestRetryOnAnyErrorStopsPromptlyWhenContextCancelled(t *testing.T) {
	...
	attempts := 0
	start := time.Now()
	err := RetryOnAnyError(ctx, discardLogger(), backoff, "reach the api server", func() error {
		attempts++
		cancel()
		return fmt.Errorf("api-server down")
	})
...
func TestRetryOnAnyErrorReportsLastErrorAlongsideCancellation(t *testing.T) {
	...
	err := RetryOnAnyError(ctx, discardLogger(), fastBackoff(5), "reach the api server", func() error {
		cancel()
		return fmt.Errorf("api-server down")
	})
...
func TestCordonNodeDoesNotRetryOnItsOwn(t *testing.T) {
...
func TestUncordonNodeDoesNotRetryOnItsOwn(t *testing.T) {
